...
version_added: "2.8"
short_description: Manage Redis instance.
description:
    - Create, update and delete instance of Redis.
    ...
    def exec_module(self, **kwargs):
        ...
        for key in list(self.module_arg_spec.keys()) + ['tags']:
            if hasattr(self, key):
                ...
                if key == "redis_configuration":
                    self.parameters["redis_configuration"] = kwargs[key]
                elif key == "enable_non_ssl_port":
                    self.parameters["enable_non_ssl_port"] = kwargs[key]
                elif key == "tenant_settings":
                    self.parameters["tenant_settings"] = kwargs[key]
                elif key == "shard_count":
                    self.parameters["shard_count"] = kwargs[key]
                elif key == "minimum_tls_version":
                    self.parameters["minimum_tls_version"] = kwargs[key]
                elif key == "sku":
                    ev = kwargs[key]
                    if 'name' in ev:
                        if ev['name'] == 'basic':
