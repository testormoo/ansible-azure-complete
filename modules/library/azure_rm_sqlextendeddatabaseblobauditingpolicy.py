...
version_added: "2.8"
short_description: Manage Extended Database Blob Auditing Policy instance.
description:
    - Create, update and delete instance of Extended Database Blob Auditing Policy.
    ...
    is_azure_monitor_target_enabled:
        description:
            ...
            - "When using REST API to configure auditing, Diagnostic Settings with 'SQLSecurityAuditEvents' diagnostic logs category on the database should
               be also created.\n"
            - "Note that for server level audit you should use the 'master' database as <I(database_name)>.\n"
            ...
            - "PUT
              ...
              atabases/<I(database_name)>/providers/microsoft.insights/diagnosticSettings/<settingsName>?api-version=2017-05-01-preview\n"
...
class AzureRMExtendedDatabaseBlobAuditingPolicies(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        super(AzureRMExtendedDatabaseBlobAuditingPolicies, self).__init__(derived_arg_spec=self.module_arg_spec,
    ...
    def exec_module(self, **kwargs):
        ...
        if not old_response:
            ...
            if self.state == 'absent':
                ...
                self.log("Need to check if Extended Database Blob Auditing Policy instance has to be deleted or may be updated")
        ...
        if (self.to_do == Actions.Create) or (self.to_do == Actions.Update):
            ...
            while self.get_extendeddatabaseblobauditingpolicy():
                time.sleep(20)
        else:
    ...
    def create_update_extendeddatabaseblobauditingpolicy(self):
        ...
        '''
        self.log("Creating / Updating the Extended Database Blob Auditing Policy instance {0}".format(self.blob_auditing_policy_name))
    ...
    def delete_extendeddatabaseblobauditingpolicy(self):
        ...
        :return: True
        '''
        self.log("Deleting the Extended Database Blob Auditing Policy instance {0}".format(self.blob_auditing_policy_name))
        try:
    ...
    def get_extendeddatabaseblobauditingpolicy(self):
        ...
        :return: deserialized Extended Database Blob Auditing Policy instance state dictionary
        '''
        self.log("Checking if the Extended Database Blob Auditing Policy instance {0} is present".format(self.blob_auditing_policy_name))
        found = False
        try:
...
def main():
    """Main execution"""
    AzureRMExtendedDatabaseBlobAuditingPolicies()
