...
DOCUMENTATION = '''
---
module: azure_rm_storagesyncsyncgroup_facts
version_added: "2.8"
    ...
    contains:
        id:
            ...
            sample: "/subscriptions/3a048283-338f-4002-a9dd-a50fdadcb392/resourceGroups/SampleResourceGroup_1/providers/Microsoft.StorageSync/storageSyncServ
                    ices/SSS_Restore_08-08_Test112/syncGroups/SampleSyncGroup_1"
        name:
            description:
                - The name of the resource
            returned: always
            type: str
            sample: SampleSyncGroup_1
'''
...
class AzureRMSyncGroupFacts(AzureRMModuleBase):
    ...
    def list_by_storage_sync_service(self):
        ...
        results = []
        try:
            response = self.mgmt_client.sync_groups.list_by_storage_sync_service(resource_group_name=self.resource_group,
                                                                                 storage_sync_service_name=self.storage_sync_service_name)
            self.log("Response : {0}".format(response))
        except CloudError as e:
            self.log('Could not get facts for Sync Group.')
