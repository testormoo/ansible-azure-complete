#!/usr/bin/python
#
# Copyright (c) 2018 Zim Kalinowski, <zikalino@microsoft.com>
#
# GNU General Public License v3.0+ (see COPYING or https://www.gnu.org/licenses/gpl-3.0.txt)

from __future__ import absolute_import, division, print_function
__metaclass__ = type


ANSIBLE_METADATA = {'metadata_version': '1.1',
                    'status': ['preview'],
                    'supported_by': 'community'}


DOCUMENTATION = '''
---
module: azure_rm_operationsmanagementmanagementconfiguration_facts
version_added: "2.8"
short_description: Get Azure Management Configuration facts.
description:
    - Get facts of Azure Management Configuration.

options:
    resource_group:
        description:
            - The name of the resource group to get. The name is case insensitive.
    name:
        description:
            - User Management Configuration Name.

extends_documentation_fragment:
    - azure

author:
    - "Zim Kalinowski (@zikalino)"

'''

EXAMPLES = '''
  - name: Get instance of Management Configuration
    azure_rm_operationsmanagementmanagementconfiguration_facts:
      resource_group: resource_group_name
      name: management_configuration_name

  - name: List instances of Management Configuration
    azure_rm_operationsmanagementmanagementconfiguration_facts:
'''

RETURN = '''
management_configurations:
    description: A list of dictionaries containing facts for Management Configuration.
    returned: always
    type: complex
    contains:
        id:
            description:
                - Resource ID.
            returned: always
            type: str
            sample: /subscriptions/subid/resourceGroups/rg1/providers/Microsoft.OperationsManagement/ManagementConfigurations/managementConfiguration1
        name:
            description:
                - Resource name.
            returned: always
            type: str
            sample: managementConfiguration1
        location:
            description:
                - Resource location
            returned: always
            type: str
            sample: East US
        properties:
            description:
                - Properties for ManagementConfiguration object supported by the OperationsManagement resource provider.
            returned: always
            type: complex
            sample: properties
            contains:
                parameters:
                    description:
                        - Parameters to run the ARM template
                    returned: always
                    type: complex
                    sample: parameters
                    contains:
                template:
                    description:
                        - The Json object containing the ARM template to deploy
                    returned: always
                    type: str
                    sample: "{\n  '$schema': 'https://schema.management.azure.com/schemas/2015-01-01/deploymentTemplate.json',\n  'contentVersion':
                             '1.0.0.0',\n  'parameters': {\n    'parentResourceName': {\n      'type': 'string'\n    },\n    'managementConfigurationName':
                             {\n      'type': 'string'\n    },\n    'jsonobject': {\n      'type': 'object'\n    }\n  },\n  'resources': [\n    {\n
                             'apiVersion': '2015-11-01-preview',\n      'name': '[concat(parameters('parentResourceName'), '/',
                             parameters('managementConfigurationName'), '-', parameters('jsonobject').displayName)]',\n      'type':
                             'Microsoft.OperationalInsights/workspaces/savedsearches',\n      'dependsOn': [],\n      'properties': {\n        'ETag':
                             '*',\n        'Category': 'A-Templated',\n        'DisplayName': '[parameters('jsonobject').displayName]',\n        'Query':
                             '[parameters('jsonobject').query]',\n        'Version': '1'\n      }\n    }\n  ],\n  'outputs': {}\n}"
'''

from ansible.module_utils.azure_rm_common import AzureRMModuleBase

try:
    from msrestazure.azure_exceptions import CloudError
    from azure.mgmt.operationsmanagement import OperationsManagementClient
    from msrest.serialization import Model
except ImportError:
    # This is handled in azure_rm_common
    pass


class AzureRMManagementConfigurationFacts(AzureRMModuleBase):
    def __init__(self):
        # define user inputs into argument
        self.module_arg_spec = dict(
            resource_group=dict(
                type='str'
            ),
            name=dict(
                type='str'
            )
        )
        # store the results of the module operation
        self.results = dict(
            changed=False
        )
        self.mgmt_client = None
        self.resource_group = None
        self.name = None
        super(AzureRMManagementConfigurationFacts, self).__init__(self.module_arg_spec, supports_tags=False)

    def exec_module(self, **kwargs):
        for key in self.module_arg_spec:
            setattr(self, key, kwargs[key])
        self.mgmt_client = self.get_mgmt_svc_client(OperationsManagementClient,
                                                    base_url=self._cloud_environment.endpoints.resource_manager)

        if (self.resource_group is not None and
                self.name is not None):
            self.results['management_configurations'] = self.get()
        else:
            self.results['management_configurations'] = self.list_by_subscription()
        return self.results

    def get(self):
        response = None
        results = []
        try:
            response = self.mgmt_client.management_configurations.get(resource_group_name=self.resource_group,
                                                                      management_configuration_name=self.name)
            self.log("Response : {0}".format(response))
        except CloudError as e:
            self.log('Could not get facts for Management Configuration.')

        if response is not None:
            results.append(self.format_response(response))

        return results

    def list_by_subscription(self):
        response = None
        results = []
        try:
            response = self.mgmt_client.management_configurations.list_by_subscription()
            self.log("Response : {0}".format(response))
        except CloudError as e:
            self.log('Could not get facts for Management Configuration.')

        if response is not None:
            for item in response:
                results.append(self.format_response(item))

        return results

    def format_response(self, item):
        d = item.as_dict()
        d = {
            'resource_group': self.resource_group,
            'id': d.get('id', None),
            'name': d.get('name', None),
            'location': d.get('location', None),
            'properties': {
                'parameters': {
                },
                'template': d.get('properties', {}).get('template', None)
            }
        }
        return d


def main():
    AzureRMManagementConfigurationFacts()


if __name__ == '__main__':
    main()
