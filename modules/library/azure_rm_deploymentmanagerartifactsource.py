...
options:
    ...
    artifact_root:
        description:
            - "The path from the location that the 'I(authentication)' property [say, a SAS URI to the blob container] refers to, to the location of the
               artifacts. This can be used to differentiate different versions of the artifacts. Or, different types of artifacts like binaries or
               templates. The location referenced by the I(authentication) property concatenated with this optional artifactRoot path forms the artifact
...
class AzureRMArtifactSource(AzureRMModuleBase):
    ...
    def exec_module(self, **kwargs):
        ...
        for key in list(self.module_arg_spec.keys()) + ['tags']:
            if hasattr(self, key):
                ...
                self.artifact_source_info[key] = kwargs[key]
        ...
        if not old_response:
            ...
            if self.state == 'absent':
                ...
                if (not default_compare(self.artifact_source_info, old_response, '', self.results)):
